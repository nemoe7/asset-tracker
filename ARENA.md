...
- Ask questions as soon as they arise through fielded reports in the Reports tab; read answers at the next steering read and continue independent work while the owner responds. Use `ask_user` only if the user explicitly requests the question tool, the preview is unavailable (including failed publication), or ntfy is selected. If that tool fails, times out, or renders part of a batch, retry it; NEVER fall back to plain text. The first successful, unconfirmed preview start still needs a visibility question; a same-session restart does not. Every question needs a recommended answer, marked among options where offered.
