...
3. Imports are idempotent by record ID. Note/answer imports preserve supplied text, timestamps, read stamps and complete receipts verbatim, plus task links. Missing receipts stay unacknowledged; partial receipts and conflicting existing text are rejected. Existing IDs keep their stored receipts. Never infer an answer from a pasted log or bulk-ack imported notes. The import does not fetch ntfy messages.
