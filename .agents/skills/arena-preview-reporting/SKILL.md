...
description: Publish readable Markdown reports in Arena.ai's live preview, with a Reports tab, multiple named reports, stable updates and portable standalone HTML files. Use for longer reports or requested report artifacts in Arena Agent Mode; keep short reports in chat. Reuses the sibling arena-preview-steering server, never a second server or duplicated runtime.
license: MIT
compatibility: Arena.ai Agent Mode, Python 3.10+, markdown-it-py and the sibling arena-preview-steering skill. HTML exports need only a browser to read.
metadata:
...
Browser download/source controls are intentionally absent: the owner observed silent failures despite HTTP 200 attachment responses. The exact browser restriction is unconfirmed. Keep the Markdown source; export HTML from the agent side:

```bash
python <steering-skill>/scripts/preview.py --state-dir reports/arena-preview export review reports/review.html
```

HTML embeds its stylesheet and dark/light toggle. Reading it needs only a browser, not this repository, server, Python or renderer. Linked documents are not bundled. Open the HTML artifact with the file-viewer tool; if it shows raw source, report that rather than claiming rendering worked. The exported file can be opened in a browser. Do not silently restore the unreliable attachment links.

Keep Markdown sources, exports, session databases, inboxes and receipts **ignored and uncommitted; never push them**. This replaces the former local-only report commits used to expose raw Markdown in Arena's diff viewer. The channel is not guaranteed permanent: report preview failure and agree on a replacement, never silently reinstate the old workaround. See [report format and recovery](references/REFERENCE.md).
